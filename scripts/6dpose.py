...
from pose6d.registrate import register_instance, RegistratedData
...
def main():
    ...
    # Registro
    reg_result: RegistratedData = register_instance(
        loader, config, scene_id, img_id, inst_idx
    )
    print(f"Scene pts: {len(reg_result.scene_pts)}")
    print(
        f"Object pts: {len(reg_result.masked_points) if reg_result.masked_points is not None else 'N/A'}"
    )
