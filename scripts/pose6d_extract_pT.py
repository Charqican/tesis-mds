...
from pose6d.data_loader import LMOLoader, InstanceData
...
SYMMETRY_DIR = CACHE_ROOT / "features_symmetry_scalar"

# TODO: confirmar los obj_id reales de los dos objetos simétricos de LM-O
SYMMETRIC_OBJ_IDS = {10, 11}  # placeholder -- ajustar a tus ids reales

MIN_VISIB_FRACT = 0.05  # descarta instancias casi totalmente ocluidas / sin señal útil
...
def extract_scene(
    ...
    n_saved = 0
    img_ids = loader.scene_image_ids(scene_id)
    ...
    for img_id in img_ids:
        ...
        for inst_idx, instance in enumerate(instances):
            ...
            np.savez(
                ...
                diameter=np.float32(diameters[instance.obj_id]),
                obj_id=instance.obj_id,
                scene_id=scene_id,
                img_id=img_id,
                inst_idx=inst_idx,
                visib_fract=(
                    instance.visible_fract
                    if instance.visible_fract is not None
                    else -1.0
                ),
                R=instance.R.astype(np.float32),  # NUEVO
                t=instance.t.astype(np.float32),  # NUEVO
            )
            n_saved += 1
...
def main():
    ...
    registrate_logger.info(f"Objetos simétricos detectados: {target_obj_ids}")

    n = extract_scene(loader, config, config.default_scene, target_obj_ids, diameters)
    registrate_logger.info(f"Listo. {n} instancias guardadas en {POINTS_PT_DIR}")

    # registration Info
    counts = Counter()
    visib_fracts = []
    for f in Path("/mnt/data/dev/dataset/tesis/6dpose/cache/points_pT").glob("*.npz"):
        data = np.load(f)
        counts[int(data["obj_id"])] += 1
        visib_fracts.append(float(data["visib_fract"]))
